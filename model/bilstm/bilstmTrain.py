...
from bilstm import BiLSTM, full_connect_layer, ModelConfig
import collections
import re
import os
import codecs

logger_file = './output/main.log'
...
def create_model(model_config, is_training, input_ids, input_length, labels, num_labels):
    # 创建模型计算输出结果
    model = BiLSTM(config=model_config, is_training=is_training,
...
def model_fn_builder(model_config, num_labels, init_checkpoint, init_lr, num_train_steps, num_warmup_steps):
    def model_fn(features, labels, mode, params):
        ...
        labels_ids = tf.squeeze(labels, axis=-1)  # label从
        ...
        (total_loss, logits, probabilities) = create_model(
            ...
            labels_ids,
            num_labels)
        pre_labels = tf.math.argmax(probabilities, axis=-1)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            output_spec = tf.estimator.EstimatorSpec(
                mode, loss=total_loss, eval_metric_ops=metric_fn(labels_ids, pre_labels))
        else:
            output_spec = tf.estimator.EstimatorSpec(
...
def train():
    ...
    num_train_steps = num_train_examples * FLAGS.batch_size
...
def argument_parse():
    ...
    tf.app.flags.DEFINE_boolean(
        'do_predict', False, 'Whether to do prediction with the model?')
    tf.app.flags.DEFINE_boolean(
    ...
    tf.app.flags.DEFINE_integer(
        'save_summary_steps', 5, 'The steps to save summary of the model')
    tf.app.flags.DEFINE_integer(
        'save_checkpoints_steps', 5, 'The steps to save checkpoint of the model')
    ...
    tf.app.flags.DEFINE_float(
        'num_warmup_proportion', 0.2, 'The warmup steps')

    tf.app.flags.DEFINE_string(
        'train_file_path', '/home/qinyuan/MyProject/classification/z_experinment/dataset/example.tfrecords', 'The path of train dataset.')
    tf.app.flags.DEFINE_string(
        'test_file_path', '', 'The path of test dataset.')
    tf.app.flags.DEFINE_string(
        'eval_file_path', '/home/qinyuan/MyProject/classification/z_experinment/dataset/example.tfrecords', 'The path of dev dataset.')
    tf.app.flags.DEFINE_string(
        'size_file_path', '/home/qinyuan/MyProject/classification/z_experinment/dataset/size.txt', 'The path of size file.')
    ...
    tf.app.flags.DEFINE_string(
        'json_file', '/home/qinyuan/MyProject/classification/z_experinment/model/init_checkpoint/model_config.json', 'The path of params json file.')
    tf.app.flags.DEFINE_integer(
