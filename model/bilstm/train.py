...
from bilstm import ModelConfig, BiLSTM,full_connect_layer
import collections
import re
import os
import codecs
...
def create_model(model_config, is_training, input_ids, input_length, labels, num_labels):
    ...
    with tf.variable_scope("loss"):
        ...
        if labels is not None:
            ...
            total_loss = tf.losses.softmax_cross_entropy(onehot_labels=one_hot_labels,logits=logits)
        else:
            total_loss = None
...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps=10):
    ...
    grads, global_norm = tf.clip_by_global_norm(gradients, clip_norm=50.0)  #
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            summary_hook = tf.train.SummarySaverHook(
                save_steps=50, output_dir=os.path.join(FLAGS.output_dir, 'summary'), summary_op=summary_op)
            ...
            output_spec = tf.estimator.EstimatorSpec(
                mode, loss=total_loss, eval_metric_ops=metric_fn(labels_ids, pre_labels), train_hooks=[logging_hook])
        else:
            output_spec = tf.estimator.EstimatorSpec(
...
def argument_parse():
    ...
    tf.app.flags.DEFINE_integer(
        'save_summary_steps', 50, 'The steps to save summary of the model')
    tf.app.flags.DEFINE_integer(
        'save_checkpoints_steps', 1000, 'The steps to save checkpoint of the model')
    ...
    tf.app.flags.DEFINE_integer(
        'train_epochs', 5, 'The epochs of training the model')
    tf.app.flags.DEFINE_integer(
    ...
    tf.app.flags.DEFINE_string(
        'json_file', '/home/qinyuan/MyProject/classification/model/bilstm/init_checkpoint/model_config.json', 'The path of params json file.')
    tf.app.flags.DEFINE_integer(
